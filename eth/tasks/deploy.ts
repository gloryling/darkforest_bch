...
task('deploy', 'deploy all contracts')
  ...
  .addOptionalParam('fund', 'amount of eth to fund whitelist contract for fund', 0.05, types.float) //0.5, types.float)
  .addOptionalParam(
    'subgraph',
...
async function deploy(
  ...
  if (typeof args.whitelist === 'undefined') {
    ...
    whitelistEnabled = false;//isDev ? false : true;
  } else {
  ...
  const requires = hre.ethers.utils.parseEther('0.2');//('2.1');
...
export async function deployAndCut(
  ...
  const initTx = await diamondCut.diamondCut(toCut, hre.ethers.constants.AddressZero, '0x', {gasLimit:'0x4C4B40'});
  const initReceipt = await initTx.wait();
  if (!initReceipt.status) {
    throw Error(`Diamond cut failed: ${initTx.hash}`);
  }
  console.log('Diamond cut ok')

  const initFunctionCall2 = diamondInit.interface.encodeFunctionData('initBefore');
  const initTx2 = await diamondCut.diamondCut([], initAddress, initFunctionCall2, {gasLimit:'0x4C4B40'});
  const initReceipt2 = await initTx2.wait();
  if(!initReceipt2.status){
    throw Error(`Diamond cut2 failed: ${initTx2.hash}`);
  }
  console.log('initBefore ok')

  const initTx3 = await diamondCut.diamondCut([], initAddress, initFunctionCall, {gasLimit:'0x4C4B40'});
  const initReceipt3 = await initTx3.wait();
  if(!initReceipt3.status){
    throw Error(`Diamond cut2 failed: ${initTx3.hash}`);
  }
  console.log('init ok')

  const initFunctionCall4 = diamondInit.interface.encodeFunctionData('initAfter');
  const initTx4 = await diamondCut.diamondCut([], initAddress, initFunctionCall4, {gasLimit:'0x4C4B40'});
  const initReceipt4 = await initTx4.wait();
  if(!initReceipt2.status){
    throw Error(`Diamond cut2 failed: ${initTx2.hash}`);
  }
  console.log('initAfter ok')
  console.log('Completed diamond cut');

  return [diamond, diamondInit, initReceipt2] as const;
}
