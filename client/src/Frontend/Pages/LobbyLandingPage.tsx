...
class LobbyPageTerminal {
  ...
  public async chooseAccount() {
    ...
    this.terminal.println(`which owns at least 0.25 bch.`);
    this.terminal.newline();
  ...
  private async displayAccounts() {
    ...
    for (let i = 0; i < accounts.length; i += 1) {
      ...
      if (this.balancesEth[i] < 0.25) {
        this.terminal.println(this.balancesEth[i].toFixed(2) + ' bch', TerminalTextStyle.Red);
      } else {
        this.terminal.println(this.balancesEth[i].toFixed(2) + ' bch', TerminalTextStyle.Green);
      }
    }
    this.terminal.println(``);
    ...
    if (isNaN(selection) || selection > accounts.length) {
      ...
      this.terminal.println('Not enough bch. Select another account.', TerminalTextStyle.Red);
      await this.displayAccounts();
    } else {
